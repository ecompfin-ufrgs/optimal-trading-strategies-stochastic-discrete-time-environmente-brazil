...
class Investidor:
    ...
    def __init__(self, gamma: float, beta: float, w0: float, horizonte: int) -> None:
        if gamma <= 0:
            raise ValueError("gamma (aversao ao risco) deve ser > 0.")
        if not 0.0 < beta < 1.0:
    ...
    def carteira_otima(self, mercado: RendaVariavel, rf: float, *,
        ...
        O n_scenarios padrao e o da base mensal (200 mil) e serve pra uso
        direto num notebook; pela esteira quem escolhe o numero e o
        app.principal, pela frequencia dos dados. Numa base diaria 200 mil e
    ...
    def fracoes_consumo(self) -> np.ndarray:
        """Fracoes de consumo theta_t = A_t^(-1/gamma), t=0..T. (F8, F9)

        Requer carteira_otima(...) chamado antes (usa o Phi_chapeu guardado).
        """
        if self._phi_hat is None:
            raise RuntimeError(
                "chame carteira_otima() antes de fracoes_consumo(): as "
                "fracoes de consumo dependem do Phi, que sai da carteira."
            )
    ...
    def alpha_star(self) -> np.ndarray | None:
        """Ultima carteira otima alpha* calculada (ou None)."""
        return self._alpha_star

    @property
    def phi_hat(self) -> float | None:
        """Phi_chapeu da ultima politica otima (ou None)."""
    ...
    def coeficientes_A(self) -> np.ndarray | None:
        ...
        Ficam guardados porque a funcao valor (F11) precisa deles depois.
        """
        return self._A
